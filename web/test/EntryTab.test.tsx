...
describe('EntryTab', () => {
  it('posts a new entry with the default winner and clears the form', async () => {
    ...
    await user.selectOptions(screen.getByLabelText('Boulder competitor'), '101')
    await user.selectOptions(screen.getByLabelText('Denver competitor'), '201')
  ...
  it('blocks save on a tie until a winner is picked, then sends a decision', async () => {
    ...
    mount()
    const user = userEvent.setup()
    await user.selectOptions(screen.getByLabelText('Boulder competitor'), '100')
    await user.selectOptions(screen.getByLabelText('Denver competitor'), '200')
  ...
  it('offers pending pairs and a start banner in setup', async () => {
    ...
    expect(screen.getByLabelText('Boulder competitor')).toHaveValue('101')
    expect(screen.getByLabelText('Denver competitor')).toHaveValue('201')
  ...
  it('tabs through the form in the order spec 9.2 requires', async () => {
    ...
    const kidA = screen.getByLabelText('Boulder competitor')
    const kidB = screen.getByLabelText('Denver competitor')
