...
describe('LiveTab', () => {
  it('shows connect info, mat status from the stream, and posts a reopen', async () => {
    const feed = snapshotFeed(played)
    ...
    const row = await screen.findByRole('row', { name: /Mat 1/ })
  ...
  it('numbers matches by the running order, not by their place in the table', async () => {
    ...
    const feed = snapshotFeed(sampleSnapshot({ mats: [{ id: 1, number: 1, current: null, onDeck: [], bound: false }], matches: [pending, second] }))
    mount(url => feed.handle(url) ?? connectOnly(url))
    expect(await screen.findByRole('row', { name: /Match 2/ })).toBeInTheDocument()
  ...
  it('edits a finished result with the picked winner', async () => {
    ...
    const matchRow = await screen.findByRole('row', { name: /Match 1/ })
    await user.click(within(matchRow).getByRole('button', { name: 'Edit result' }))
