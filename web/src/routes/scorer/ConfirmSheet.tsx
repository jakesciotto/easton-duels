...
function WinnerToggle({ name, color, pressed, onClick }: { name: string; color: string; pressed: boolean; onClick: () => void }) {
  return (
    <button
      ...
      onClick={onClick}
      className="touch inline-flex h-14 items-center justify-center gap-2 rounded-md text-base font-medium text-soft shadow-[0_0_0_1px_#2f3037] transition-[color,background-color,box-shadow] duration-150 focus-visible:shadow-focus aria-pressed:bg-secondary aria-pressed:text-foreground aria-pressed:shadow-ring"
    >
      <TeamDot color={color} />
      <span>{name} wins</span>
    </button>
