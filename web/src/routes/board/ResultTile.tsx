...
export function ResultTile({ match, teams, large = false }: { match: MatchView; teams: TeamView[]; large?: boolean }) {
  return (
    <section
      ...
      className="grid grid-rows-[auto_1fr_1fr] gap-[calc(0.5vw*var(--scale,1))] rounded-[1vw] border border-ok/45 bg-card py-[calc(1vw*var(--scale,1))] px-[calc(1.3vw*var(--scale,1))]"
    >
      <div className="text-[calc(1vw*var(--scale,1))] text-faint">Match {match.orderIndex + 1}</div>
