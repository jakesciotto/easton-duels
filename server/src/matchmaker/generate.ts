...
export async function generateMatches(db: DbLike, eventId: number): Promise<GenerateResult> {
  return db.transaction(async tx => {
    ...
    const ruleset = await tx.select().from(rulesets).where(eq(rulesets.eventId, eventId)).orderBy(asc(rulesets.id)).get()
    if (!ruleset) throw new MatchStateError('event needs a ruleset')
    const all = await tx.select().from(athletes).where(eq(athletes.eventId, eventId)).all()
    const A = all.filter(a => a.teamId === teamRows[0].id)
    const B = all.filter(a => a.teamId === teamRows[1].id)

    const stale = (await tx.select({ id: matches.id }).from(matches)
      .where(and(eq(matches.eventId, eventId), eq(matches.status, 'pending'))).all()).map(m => m.id)
    if (stale.length) {
      await tx.update(mats).set({ currentMatchId: null }).where(inArray(mats.currentMatchId, stale)).run()
    ...
    const matRows = await tx.select().from(mats).where(eq(mats.eventId, eventId)).orderBy(asc(mats.number)).all()
