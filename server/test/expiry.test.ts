...
describe('ExpiryScheduler', () => {
  ...
  it('sync schedules a running match and cancels a paused one; rebuild scans the db', () => {
    vi.useFakeTimers({ now: T0 })
    const db = freshDb()
    const s = seedEvent(db, { live: true })
    const sched = new ExpiryScheduler(() => {})
    const running = appendMatchEvent(db, { id: 'c1', matchId: s.matchIds[0], type: 'clock_start', lastSeq: 0, at: iso(T0) }).match
    sched.sync(running)
    expect(sched.pendingCount()).toBe(1)
    const paused = appendMatchEvent(db, { id: 'c2', matchId: s.matchIds[0], type: 'clock_pause', lastSeq: 1, at: iso(T0 + 1_000) }).match
    sched.sync(paused)
    expect(sched.pendingCount()).toBe(0)
    appendMatchEvent(db, { id: 'c3', matchId: s.matchIds[0], type: 'clock_start', lastSeq: 2, at: iso(T0 + 2_000) })
...
describe('expireClock', () => {
  it('writes a pause at the expiry instant and caps elapsed at the length', () => {
    const db = freshDb()
    const s = seedEvent(db, { live: true })
    appendMatchEvent(db, { id: 'c1', matchId: s.matchIds[0], type: 'clock_start', lastSeq: 0, at: iso(T0) })
    const m = expireClock(db, s.matchIds[0], iso(T0 + 300_000))
    expect(m?.clockStartedAt).toBeNull()
    expect(m?.clockElapsedMs).toBe(300_000)
    expect(loadEvents(db, s.matchIds[0]).at(-1)?.at).toBe(iso(T0 + 300_000))
  })

  it('does nothing when the clock is already paused or the match is done', () => {
    const db = freshDb()
    const s = seedEvent(db, { live: true })
    expect(expireClock(db, s.matchIds[0], iso(T0))).toBeNull()
    expect(loadMatch(db, s.matchIds[0]).lastSeq).toBe(0)
  })
})
